...
- [~] Phone access over Tailscale from any network, with a guided iPhone setup (QR for every step, retry on failure), a phone app that shows a readable error instead of a blank screen, and a settings panel with real navigation instead of one long scroll. <!-- conductor-task:phone-tailscale-setup-and-settings-nav agent=agent_mudb8nhm_wvc846v -->
