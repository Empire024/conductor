...
- [~] Local-model turns survive a Conductor restart, as native runtimes now do (372902d, docs/runtime-host.md "Not covered (yet)"). The llama.cpp server already outlives the app, but the local agent loop (src/main/local-models/agent.ts) runs in the Electron main process, so a running local turn stops on restart or update. Move the loop into the runtime host, or checkpoint it at a round boundary and resume it after reattach using the durable TaskState. The turn must continue in its tab with no lost rounds. <!-- conductor-task:local-turns-survive-restart agent=agent_mufzmkdy_58p1hds -->
