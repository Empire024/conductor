...
- [x] Codex Auto mode refused every MCP tool after the 0.155 rebaseline ("MCP tool call requires approval, but approval policy is never"): the project browser, chrome-devtools and node_repl were exposed but never allowed to run. In Auto the thread now starts with every enabled, unset MCP server cleared to run unasked (explicit config.toml modes kept, nothing written to the CLI configuration), the same native conversation reconnects when the mode crosses that boundary, the adapter says so when a never-ask turn reaches a thread connected in an asking mode, and the Auto description states that MCP tools run without approval. Live-verified against codex-cli 0.155.1; see docs/codex-compatibility.md. <!-- conductor-task:codex-auto-mcp-approval agent=agent_muco6clt_yrobrp3 -->

- [x] Browser toggle persistence: the owner's explicit browser-tools choice is remembered per provider across restarts and applied to every new conversation of that provider, including tabs a coworker or the router opens; existing conversations keep their own choice. <!-- conductor-task:browser-toggle-memory agent=agent_muco6clt_yrobrp3 -->

- [~] `helper_unknown_error: setup refresh had errors` returned on 2026-09-22 with a new cause outside Conductor: the Codex desktop app installed a runtime whose pnpm cache holds a path over MAX_PATH, and Codex's Windows sandbox helper fails the whole refresh on it, so no Codex shell starts in any project. Conductor now explains it once in the conversation; the repair itself is the owner's call (docs/codex-windows-sandbox-repair.md). <!-- conductor-task:codex-sandbox-long-path agent=agent_muco6clt_yrobrp3 -->
