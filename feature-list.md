...
- [ ] Token totals are inflated by repeated context. Jobs and processes show figures like 61M / 56M / 50M tokens for one worker. That total re-counts the whole context on every turn: each turn's input includes cache reads of the same prefix, and `totalTokens` falls back to input + output (src/shared/usage-accounting.ts:23, src/shared/weekly-model-usage.ts:82) and is summed per turn (ProcessDashboardPane.tsx:109/173). Fix, low cost (a Sonnet/Haiku-sized change, schedule after the swarm):
...
- [ ] Finished coworker tabs must close themselves, not pile up. On 2026-09-25 the owner found 30 coworker tabs in one workspace, each keeping a `claude` CLI alive (36 processes, 10 GB), with the Processes board showing hours-old finished workers as "Working". Rules:
