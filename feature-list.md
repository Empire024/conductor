...
- [ ] Exiting Conductor still shows tabs that are actually inactive & have been for a while i.e. tasks that are not running currently.. <!-- conductor-task:19c298e4-2bea-4164-bba3-25d73bba22d4 agent=agent_mufv9yop_1m4tetm -->
...
- [ ] # Build: Conductor Ideas / Notes System <!-- conductor-task:9b15feab-3e3b-4e9a-b23a-76ce29538e4d weight=heavy agent=agent_mufylflq_vx3d57o -->
...
- [ ] App control must accept concurrent requests from one session. src/main/agent-control-server.ts:90-93 answers 409 "This session already has a control request in progress" to any second call. So a controller holding a 100 s `git.ship.status` or `app.update.status` wait cannot list agents, open a tab or steer a worker until it returns: the 2026-09-24 swarm orchestrator was blocked from opening a local worker while a ship was in flight. Read-only and waiting methods (`*.list`, `*.status`, `agents.snapshot/history/status`, `usage.limits`, `models.list`, `app.state`, `tools.list`, `git.status`, `git.ship.status`, `app.update.status`, `jobs.*` reads) run concurrently. Mutations stay serialized per session and per target, so the "no racing its own calls" guarantee stays where it matters (a per-(session, method family) lock rather than a per-session lock). Add a cap of e.g. 8 in-flight requests per session and tests for both paths. Update the comment and docs/agent-control.md. <!-- conductor-task:control-concurrent-requests priority=high -->
