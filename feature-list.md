...
- [ ] Exiting Conductor still shows tabs that are actually inactive & have been for a while i.e. tasks that are not running currently.. <!-- conductor-task:19c298e4-2bea-4164-bba3-25d73bba22d4 -->
...
- [ ] archive tasks that are longer than 14 days old so we don't clutter our project tasks view (already very large) <!-- conductor-task:3bbf4893-c6c1-4b90-91a4-a7b21bfeaf4d -->
...
- [ ] To-do's don't show which is priority and which is weight, make sure it does on hover at least in project tasks view. <!-- conductor-task:5a68facc-0abc-49c4-b0ae-8063644d4dc6 priority=low weight=light -->

- [ ] Projects & files left-side part needs to be expandable i.e. stretchable (so we can  read filenames project names etc etc better <!-- conductor-task:fe4662b9-e2a6-4436-a753-62e7bbed9b9c -->
...
- [ ] message needs to be able to be unsent in Conductor when still queued and not actually sent easily via clicking X or some keyboard shortcut <!-- conductor-task:2ad233c6-9e4c-416c-b622-fa4bb49acadf -->
...
- [ ] refine way we queue messages, if one is queued and we send another, they need to be send immidiately together in one turn, right now were wasting tokens and having me bash escape stopping the model altogether and having me write continue! <!-- conductor-task:9fb1c9c5-a27d-4a2c-96f7-2cfb5cd810ce -->
...
- [ ] Processes tab needs to scope all projects currently open in Conductor, but can't load this much data - it's very laggy now, loading projects from 2 weeks ago. It should realistically only show recent tabs to let us keep track of what's currently happening and churning and where and in which project and what needs attention like a little control board. <!-- conductor-task:2c712217-c17e-4bb7-8813-ba7738444230 -->

- [ ] each local model needs it's own custom icon as well to differentiate them properly <!-- conductor-task:4ab12812-f74e-438b-8c4d-c0d9cc34221e -->
...
- [ ] based on the performance updates and task list archiving etc. lets send only the tasks that make sense sending over to the phone, separate the sak list from 'new' and have a tasks section where i can see tasks per project and easily add to them. <!-- conductor-task:fb7fbf10-dfea-497e-a993-e5f3d8e63803 -->
