...
export function StructuredUsageContent({ items, truncated = false, modelLabel, agentSessionId, workspaceId }: {
  ...
  return <div className="sa-usage-content">
    ...
    {context && <section className="sa-context-details"><h4>Context window</h4><p><strong>{Math.floor(context.percent)}% used</strong> &middot; {context.used.toLocaleString()} / {context.capacity.toLocaleString()} usable tokens</p><progress max={100} value={context.percent} aria-label="Context used" />{context.reserve !== undefined && context.window !== undefined && <p>{context.window.toLocaleString()}-token window &middot; {context.reserve.toLocaleString()} reserved for the answer &middot; {context.capacity.toLocaleString()} usable for the prompt</p>}<p className="sa-detail-hint">{context.measurement ? `Measured by ${context.measurement}. ` : 'Latest context snapshot, including cached input. '}This is separate from cumulative token usage. The usable budget reflects the provider's reserved space.</p>{context.percent >= 90 && <p className="sa-context-guidance">{context.percent >= 100 ? 'Context is full.' : 'Context is nearly full.'} Use <code>/compact</code> to make room before continuing.</p>}</section>}
