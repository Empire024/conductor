...
/** Fallback wording, used when the mode list does not carry its own description. */
...
export function ConversationModeControl({ modes, value, disabled, onChange }: {
  ...
  const describe = (mode: { id: string; description?: string } | undefined): string | undefined => mode?.description ?? descriptions[mode?.id ?? '']
  ...
  return <div className="sa-mode-picker" ref={host} onBlur={event => { if (!event.currentTarget.contains(event.relatedTarget)) setOpen(false) }}>
    ...
    }}><p>Mode for your next message</p>{modes.map(mode => { const Icon = modeIcons[mode.id] ?? ShieldCheck; return <button key={mode.id} type="button" role="menuitemradio" aria-checked={value === mode.id} onClick={() => { onChange(mode.change); close() }}><Icon size={16} aria-hidden="true" /><span><strong>{mode.label}</strong><small>{describe(mode)}</small></span><span className="sa-mode-check" aria-hidden="true">{value === mode.id && <Check size={14} />}</span></button> })}</div>}
  </div>
}
