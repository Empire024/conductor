...
function fakeBridge(initial: AppUpdateState) {
  ...
  const bridge = {
    ...
    pinVersion: vi.fn(async () => { throw new Error('not used') }),
    restorePlan: vi.fn(async () => { throw new Error('not used') }),
    rollback: vi.fn(async () => { throw new Error('not used') }),
    cliPins: vi.fn(async () => []),
    useInstalledClis: vi.fn(async () => []),
    restart: vi.fn(async () => {}),
