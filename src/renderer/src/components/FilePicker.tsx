  ...
  return <AgentDialog title="Open file" onClose={onClose}>
    <div className="file-picker" onKeyDown={(event) => {
      ...
      <label className="file-picker-input"><Search size={17} /><input autoFocus placeholder="Search files in loaded projects…" aria-label="Search files"
        ...
        value={query} onChange={(event) => { setQuery(event.target.value); setSelected(0); setBusy(true); pendingSelection.current = 0; pendingOpen.current = false }} /><kbd>Ctrl E</kbd></label>
      ...
      <footer><span>↑ ↓ to navigate · Enter to open · Esc to close</span><small role="status">{busy && results.length ? 'Updating results…' : 'Up to 100 matches · Dependencies and build folders excluded'}</small></footer>
    </div>
  </AgentDialog>
}
