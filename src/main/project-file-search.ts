...
export async function searchProjectFiles(projects: ProjectRecord[], query: string): Promise<FileSearchResult[]> {
  const groups = await Promise.all(projects.map(async (project) => {
    ...
    return (await entry.paths).map((path) => ({ projectId: project.id, path, score: fileMatchScore(path, query) })).filter((item) => item.score >= 0)
  }))
