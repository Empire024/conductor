...
const markerAttrs = '(?:\\s+[a-zA-Z0-9_-]+=[a-zA-Z0-9_-]+)*'
const marker = new RegExp('\\s*<!-- conductor-task:([a-zA-Z0-9_-]+)('+markerAttrs+') -->','g')
const markerPresent = new RegExp('<!-- conductor-task:[a-zA-Z0-9_-]+'+markerAttrs+' -->')
const attr = (blob:string|undefined,name:string):string|undefined => blob ? new RegExp('(?:^|\\s)'+name+'=([a-zA-Z0-9_-]+)').exec(blob)?.[1] : undefined
/** Unknown, missing, or garbled priority markers (and case, from an agent's free text) all read back as 'normal'. */
const parsePriority = (value?:string):ProjectTaskPriority => {
...
function taskEnd(lines:string[],start:number):number {
  let end=start+1
  if(!markerPresent.test(lines[start]??''))return end
