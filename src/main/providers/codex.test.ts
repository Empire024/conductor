...
describe('Codex App Server raw synthetic process contract (zero inference)', () => {
  ...
  it('answers a native sandbox escalation itself in Auto, and still asks questions', async () => {
    const auto = create({}, undefined, 3000, { ...settings, permission: 'auto' })
    await auto.adapter.submit('synthetic:approval', { ...settings, permission: 'auto' })
    await waitFor(() => completed(auto.events))
    expect(auto.events.some(event => event.data.type === 'interaction')).toBe(false)
    expect(auto.events.some(event => event.data.type === 'session' && event.data.phase === 'waiting_approval')).toBe(false)
    expect(auto.events.some(event => event.data.type === 'notice' && event.data.message === 'Auto allowed “Synthetic harmless operation” outside the workspace sandbox without asking.')).toBe(true)
