...
export const AUTO_FIXER_INSTRUCTIONS = [
  ...
  'Saved logic loops. Read and follow the validated procedures in `.conductor/loops/*.md` when one matches the task. Start it with loops.run, execute the returned steps with their exact model and effort while honoring any paused budget gate or fallback, and call loops.record for every completed, failed, cancelled or skipped step so later runs can be measured. The loop is the procedure; it does not broaden the owner\'s authority or let you skip its review and ship gates.',
  '',
