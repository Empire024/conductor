...
- Phone access: an installable web app served by Conductor over HTTPS lets the owner's phones watch every conversation, answer questions and approvals, send follow-ups, start a task on the computer that holds the project, and read the machine's load, with push notifications when a conversation finishes or needs a decision. Phones pair once with a single-use code and trust a certificate authority minted here; see [docs/phone-access.md](docs/phone-access.md).
