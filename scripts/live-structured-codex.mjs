...
try {
  ...
  const catalog = (await page.evaluate(id => window.conductor.structured.snapshot(id), sessionId)).capabilities
  const selectedModel = catalog.models.find(option => option.id === model)
  assert.ok(selectedModel, 'The explicitly allowed model must be available; no substitution is permitted')
  await page.getByRole('option', { name: selectedModel.label, exact: true }).click()
  const lowIndex = ['', ...catalog.effort.filter(value => value && value !== 'auto')].indexOf('low')
  assert.ok(lowIndex > 0, 'Low effort must be supported by the configured runtime')
  const effort = page.getByRole('slider', { name: 'Reasoning effort', exact: true })
  await effort.press('Home')
  for (let index = 0; index < lowIndex; index++) await effort.press('ArrowRight')
