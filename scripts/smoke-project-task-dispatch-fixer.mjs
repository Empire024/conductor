...
const output = resolve('artifacts/project-task-dispatch-fixer')
...
const shot = async name => {
  ...
  await writeFile(join(output, `${name}-${evidenceId}.png`), Buffer.from(image, 'base64'), { flag: 'wx' })
}
...
try {
  ...
  const agentInput = page.getByRole('textbox', { name: 'Message Codex', exact: true })
  await agentInput.fill('synthetic:steer-wait')
  await agentInput.press('Enter')
  await expect.poll(async () => (await snapshot(existingId)).phase).toBe('running')
  await showTasks()
  ...
  assert.equal(existingState.phase, 'running')
  ...
  check('Existing assignment reaches the ongoing native turn once, then and only then records the real conversation owner')
  ...
  try { await cleanupFixtureApp(app, result, 'project task dispatch fixture cleanup') }
  catch (error) { cleanupFailure = error; result.errors.push(error.stack ?? String(error)) }
  await writeFile(join(output, `report-${evidenceId}.json`), JSON.stringify(result, null, 2), { flag: 'wx' })
