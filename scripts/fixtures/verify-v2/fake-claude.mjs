...
// V2 verify addition (scratch fixture copy only): model-ladder safeguard-refusal simulation.
const refusalLog = process.env.CONDUCTOR_TEST_REFUSAL_LOG
let currentModel = process.argv.includes('--model') ? process.argv[process.argv.indexOf('--model') + 1] : undefined
let refusalCount = 0
const logRefusal = (entry) => { if (refusalLog) writeFileSync(refusalLog, readFileSyncSafe(refusalLog) + JSON.stringify({ t: Date.now(), ...entry }) + '\n') }
function readFileSyncSafe(path) { try { return readFileSync(path, 'utf8') } catch { return '' } }
...
for await (const line of input) {
  ...
  if (message.type === 'control_request') {
    ...
    } else if (kind === 'set_model') { currentModel = message.request.model ?? currentModel; logRefusal({ event: 'set_model', model: currentModel }); success(message.request_id) }
    else if (kind === 'apply_flag_settings') success(message.request_id)
    ...
    // V2 verify addition: a merged queue turn wraps each entry as "--- Queued message N of M ---\n\n<text>",
    // so the synthetic gate also accepts a prompt that contains a SYNTHETIC line after that wrapper.
    if (typeof prompt !== 'string' || !/(?:^|--- Queued message \d+ of \d+ ---\n\n)SYNTHETIC /.test(prompt)) throw new Error('Fixture accepts explicitly synthetic prompts only')
    if (/--- Queued message \d+ of \d+ ---/.test(prompt) && !prompt.startsWith('SYNTHETIC ')) {
      logRefusal({ event: 'merged-queue-prompt', prompt })
      text('SYNTHETIC merged queue turn received: ' + (prompt.match(/SYNTHETIC [^\n]*/g) ?? []).join(' | '))
      finish(); continue
    }
    ...
    if (prompt.startsWith('SYNTHETIC REFUSE') || prompt.startsWith('SYNTHETIC ERROR500')) {
      logRefusal({ event: 'prompt', model: currentModel, prompt })
      const always = prompt.startsWith('SYNTHETIC REFUSE ALWAYS')
      const once = prompt.startsWith('SYNTHETIC REFUSE ONCE')
      const is500 = prompt.startsWith('SYNTHETIC ERROR500')
      const slow = /\bSLOW\b/.test(prompt)
      emit({ type: 'system', subtype: 'init', model: currentModel ?? 'synthetic-claude', claude_code_version: '2.1.263' })
      const shouldRefuse = is500 ? true : always ? true : once ? refusalCount === 0 : false
      const refusalTextFor = () => is500
        ? 'API Error: 500 {"type":"error","error":{"type":"overloaded_error"}}'
        : `API Error: ${currentModel === 'claude-fable-5-1' ? "Fable 5.1's" : currentModel && /opus/.test(currentModel) ? "Opus 5.5's" : "Sonnet 5's"} safeguards flagged this message (https://www.anthropic.com/legal/aup). This sometimes happens with safe, normal conversations. Claude Code can't respond to this message with ${currentModel === 'claude-fable-5-1' ? 'Fable 5.1' : currentModel && /opus/.test(currentModel) ? 'Opus 5.5' : 'Sonnet 5'}. Try rephrasing the request in a new session or change your model. Learn more: https://support.claude.com/en/articles/15363606 Details: \`[reasoning_extraction]\` Request ID: req_011CfNTWj5rcVPNDsbEz4Gqc`
      const finishRefusal = () => {
        refusalCount++
        const refusalText = refusalTextFor()
        logRefusal({ event: 'refusal', model: currentModel })
        text(refusalText)
        const useErrorDuringExecution = /SUBTYPE_EDE/.test(prompt)
        emit({ type: 'result', subtype: useErrorDuringExecution ? 'error_during_execution' : 'success', is_error: true, result: refusalText, usage: {} })
      }
      const finishNormal = () => {
        logRefusal({ event: 'answered', model: currentModel })
        text('SYNTHETIC continuation after refusal: this turn completed normally.')
        finish()
      }
      if (slow) {
        // Visible "running" turn for 4s before the outcome, so a caller can observe phase=running
        // and queue messages against a turn that has genuinely not settled yet.
        emit({ type: 'stream_event', event: { type: 'message_start', message: { id: randomUUID() } } })
        emit({ type: 'stream_event', event: { type: 'content_block_start', index: 0, content_block: { type: 'thinking', thinking: '' } } })
        emit({ type: 'stream_event', event: { type: 'content_block_delta', index: 0, delta: { type: 'thinking_delta', thinking: 'Synthetic slow refusal: thinking before responding...' } } })
        setTimeout(() => { if (shouldRefuse) finishRefusal(); else finishNormal() }, 4000)
      } else if (shouldRefuse) finishRefusal()
      else finishNormal()
      continue
    }
    ...
    if (prompt.startsWith('SYNTHETIC LONG')) {
      ...
      const batch = () => {
        for (let count = 0; count < 25 && events < total; count++, step++) {
          const tool = tools[step % tools.length], id = `long-tool-${step}`
          emit({ type: 'assistant', message: { id: `long-message-${step}`, model: 'synthetic-claude', usage: { input_tokens: 40, cache_read_input_tokens: 20_000 + step * 60, output_tokens: 300 + (step % 400) }, content: [
          ...
          result(id, outputFor(tool, step), false, tool === 'Bash' ? { stdout: outputFor(tool, step), stderr: '', exitCode: 0 } : undefined)
          events += 2
        }
        if (events < total) setTimeout(batch, 2)
        else { text(`**Synthetic long conversation:** ${events} provider messages replayed; no model or tool execution.`); finish() }
      }
      batch()
      continue
    }
