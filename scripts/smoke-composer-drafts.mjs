...
try {
  ...
  await expect(page.locator('.sa-context-chips > span')).toHaveCount(attachmentCount)
  results.checks.push('A paste too large for one attachment is refused by name and keeps the draft and its attachments')
  ...
  await page.keyboard.press('Escape')
  await page.screenshot({ path: join(output, 'pasted-text-chip.png') })
  results.checks.push('A long paste folds into an inspectable "[Pasted text #1: 612 lines]" chip with its placeholder in the message')
  ...
  const sent = (await page.evaluate(id => window.conductor.structured.snapshot(id), firstId)).items.find(item => item.data.type === 'text' && item.data.role === 'user')
  assert.equal(sent.data.text, prompt + '[Pasted text #1: 612 lines]')
  ...
  await page.reload()
  await expect(composer()).toHaveValue('')
  results.checks.push('The pasted text is sent as attached context, the timeline keeps it as an attachment rather than inline, and the send clears the draft durably')
