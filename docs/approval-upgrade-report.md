...
- A coworker keeps the mode it was opened with. `permissionMode()` in `src/main/providers/claude.ts` and the unattended branch in `src/main/providers/codex.ts` no longer look at `reviewApprovals`; only an isolated reviewer is pinned to manual.
- Every request a reviewed worker still raises is reviewable (`src/main/approval-review-gate.ts`): file mutations keep their path and prior-content precondition; commands, searches and Codex requests are reviewed as one-shot answers. `native-owner` (deny or escalate only) is reserved for an explicit Claude ask rule and for a request whose reach hits a Codex owner-only boundary (system directories, registry, elevation, services, firewall, credentials, disks/accounts, recursive deletion). Plan/read-only workers stay unsupported; protected paths still pause to the owner. The `supportsExactExecution` contract is no longer required.
- Routing (`src/main/approval-review-routing.ts`): reviews queue instead of pausing when one is running, any worker model is reviewed (the reviewer is still a runtime-discovered Opus), the budget is 400 reviews per owner task, the deadline 180 s, the owner/task evidence is bounded (newest owner message and the worker's task always kept) rather than refused, the reviewer prompt tells the reviewer to allow routine task work and escalate only what needs more owner permission, and the reviewer tab is closed after its decision.
- Journal fencing (`src/main/approval-review.ts`): a pathless denial fences only the same operation, and unobserved execution of a pathless request no longer blocks every later command of the project.

The security notes above about exact execution still describe the limits: the review binds a file's prior bytes for a Write/Edit and refuses a changed action, but it does not enforce that binding at execution time. The owner accepted that trade for their own coworkers on their own machine.
