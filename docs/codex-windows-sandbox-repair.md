...
### Quarantine performed 2026-09-22

With the owner asking for an immediate fix, the one over-long directory was moved out of the runtime cache to a short path, `%LOCALAPPDATA%\OpenAI\Codex\runtime-quarantine-2026-09-22\b474a88d5d105afa__rollup-plugin-typescript-12.1.2__bf97…` (three entries, a README beside it says how to move it back). Verification without a model: `windowsSandbox/readiness` reported `ready`, a `command/exec` through the app-server in `C:\Users\stilj\Conductor\faktury` ran `cmd /c echo SANDBOX-OK && whoami` inside the elevated sandbox (`CodexSandboxOffline`, exit code 0), the sandbox log ended in `SUCCESS`, and `setup_error.json` was gone. If the Codex desktop app reinstalls that runtime entry, the same failure returns until Codex ships a long-path-aware helper.
