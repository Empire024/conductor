...
- **A wizard tab holds the owner's authority.** The wand toggle in a frontier-model tab's composer (Claude Opus or Fable, GPT-6 Astra) makes that conversation the owner for app control: its coworkers' approvals are reviewed and answered for the owner, it may steer every tab, run `app.update`, `app.update.install` and `app.restart` without a dialog, it is brought back and told to continue after a restart Conductor started itself, and it and the coworkers it opens continue when a usage limit resets. An unattended loop over the app (`docs/overseer.md`) is what it is for.
