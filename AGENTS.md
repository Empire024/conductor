...
- **A wizard tab holds the owner's authority.** The wand toggle in a frontier-model tab's composer (Claude Opus or Fable, GPT-6 Astra) makes that conversation the owner for app control: its coworkers' approvals are reviewed and answered for the owner, it may steer every tab, run `app.update`, `app.update.install` and `app.restart` without a dialog (its own restart never raises "Work is still running"; `app.quit.confirm({stopWork})` answers one that is open), only the wizard tab that itself started the restart, or asked the owner for it with `app.restart.request({reason})`, is brought back and told to continue, and it and the coworkers it opens continue when a usage limit resets. An unattended loop over the app (`docs/overseer.md`) is what it is for. When a long wizard or controller conversation (about 25 turns or 40% context) has idle coworkers, it passes itself on with `agents.handoff({handoff, successor:true})`: the successor takes over wizard mode, every coworker and any pending restart, and the old tab turns read-only.
...
- The installed app on this machine receives finished work from the checkout through `app.update` (the local update feed shows `Update pending`), not from GitHub. A native Claude or Codex coworker in Auto runs `app.update` without the owner dialog, because a build only offers the update and installing it stays with the owner or a wizard tab; a local model, or a tab below Auto, still asks. Never tell the owner to run a development checkout, preview build, or installer to receive a completed task.
...
- `docs/machine-profile.md` records what this computer can carry: 12 GB of VRAM, one llama.cpp server at a time, four native coworkers comfortably, smokes one at a time. Read it before starting a local model, a swarm, or anything that downloads model files. App control `local.servers` shows which model server holds the GPU and who uses it, and `local.stop({model|pid})` stops one Conductor started (refused while a turn uses it unless `force:true`).
- Let the local model read for you. Claude and Codex tabs get the `conductor-local` MCP tools: for tests, builds and long logs call `run_and_summarize` (Auto only; full log kept under `.conductor-scratch/local-assist/`, last 15 lines always verbatim); for reading large files call `local_ask` or `summarize_file`. They use the running local server, never wait more than about 20 s for it, and record the tokens saved (`docs/local-assist.md`).
